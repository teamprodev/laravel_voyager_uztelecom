...
@section('center_content')
  ...
  ->post() }}
    ...
    ->enctype("multipart/form-data")
    ->post() }}
    @includeWhen($application->user_id == auth()->user()->id && $application->show_leader != 2,'site.applications.form_edit')
    ...
    @includeWhen($user->hasPermission(PermissionEnum::Warehouse) && $application->status == 'Принята'||$user->hasPermission(PermissionEnum::Warehouse) && $application->status == 'товар доставлен'||$user->hasPermission(PermissionEnum::Warehouse) && $application->status == 'товар прибыл','site.applications.warehouse')
    {{ Aire::close() }}
    @endif
@endsection
