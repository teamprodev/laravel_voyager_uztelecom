    ...
    <div class="mt-6">
        ...
        <script>
            $(function () {
                ...
                <!-- Modal toggle -->
                @php
                    $assigned_roles[] = json_decode($application->roles_need_sign, true);
                    $signed = \App\Models\SignedDocs::where('application_id', $application->id)->where('user_id', auth()->id())
                    ->first();
                @endphp
                @dd(!in_array(auth()->id(), $same_role_user_ids))
                @if((in_array(auth()->user()->role_id, $assigned_roles) && !in_array(auth()->id(), $same_role_user_ids) && !
                ($signed)))
                <button
        ...
        <form name="testform" action="{{route('site.applications.imzo.sign',$application->id)}}" method="POST">
            ...
            <button onclick="generatekey()" class="hidden btn btn-success" type="button">Подписать</button>
            <br/>
