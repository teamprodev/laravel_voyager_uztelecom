...
{{--    Modal when user click cancel button--}}

    <div id="defaultModal" aria-hidden="true" class="hidden modal-window mx-auto overflow-y-auto overflow-x-hidden fixed right-0 left-0 top-4 z-50 justify-center items-center h-modal md:h-full md:inset-0" style="background-color:rgba(0,0,0,0.5)">
        <div class="relative px-4 w-11/12 mx-auto mt-24 max-w-2xl h-full md:h-auto">
            ...
            @if(auth()->user()->role_id == 4)
                    ...
                    <form action="{{route('site.applications.vote', ['application' => $application->id])}}" method="POST">
                        <!-- Modal body -->
                        @method('put')
                        @csrf
                        ...
                        <div class="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600">
                            <button data-modal-toggle="defaultModal" type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">I accept</button>
                        </div>
                    </form>
                @endif
